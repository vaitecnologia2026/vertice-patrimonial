...
const slug = () => 'p_' + crypto.randomBytes(5).toString('hex');
const dt   = (offsetDays = 0) => new Date(Date.now() + offsetDays * 86400000);
...
async function main() {
  ...
  // ─── PARCEIROS ──────────────────────────────────────────────
  const parceiros = [
    { id:'PRC001', licId:l1.id, nome:'Lucas Andrade',   whatsapp:'(11) 99876-5432', email:'lucas@parc.com',   documento:'852.741.963-21', cidade:'São Paulo',     estado:'SP', tipoAtuacao:'Corretor de imóveis', prodInteresse:'Arrematação Individual', tipoComissao:'PERCENTUAL', valComissao:10, regraComissao:'INDICACAO',         status:'ATIVO',    origem:'lp',     slugLp:slug(), approvedAt: dt(-30), approvedBy:'admin@vertice.com.br' },
    { id:'PRC002', licId:l1.id, nome:'Sofia Martins',   whatsapp:'(11) 98765-4321', email:'sofia@parc.com',   documento:'963.852.741-12', cidade:'Campinas',      estado:'SP', tipoAtuacao:'Advogado',            prodInteresse:'Revisão Contratual',     tipoComissao:'PERCENTUAL', valComissao:15, regraComissao:'COMISSAO_RECEBIDA', status:'ATIVO',    origem:'manual', slugLp:slug(), approvedAt: dt(-22), approvedBy:'admin@vertice.com.br' },
    { id:'PRC003', licId:l1.id, nome:'Gustavo Pires',   whatsapp:'(11) 97654-3210', email:'gustavo@parc.com', documento:'741.852.963-03', cidade:'Santos',        estado:'SP', tipoAtuacao:'Indicador / Influenciador', prodInteresse:'Todos',            tipoComissao:'FIXO',       valComissao:0,                                          status:'PENDENTE', origem:'lp' },
    { id:'PRC004', licId:l1.id, nome:'Mariana Vieira',  whatsapp:'(11) 96543-2109', email:'mariana@parc.com', documento:'159.357.486-94', cidade:'São Paulo',     estado:'SP', tipoAtuacao:'Contador',            prodInteresse:'Home Equity',            tipoComissao:'PERCENTUAL', valComissao:0,                                          status:'PENDENTE', origem:'lp' },
    { id:'PRC005', licId:l3.id, nome:'Renata Oliveira', whatsapp:'(31) 92109-8765', email:'renata@parc.com',  documento:'12.345.678/0001-99', cidade:'Belo Horizonte', estado:'MG', tipoAtuacao:'Advogado',          prodInteresse:'Todos',                  tipoComissao:'PERCENTUAL', valComissao:12, regraComissao:'COMISSAO_RECEBIDA', status:'ATIVO',    origem:'manual', slugLp:slug(), approvedAt: dt(-45), approvedBy:'admin@vertice.com.br' },
  ];
  for (const p of parceiros) {
    await prisma.parceiro.upsert({ where:{id:p.id}, update:{}, create:p });
  }
  console.log(`  ${parceiros.length} parceiros`);

  // ─── PARTNER LEADS ──────────────────────────────────────────
  const leads = [
    { id:'PL001', parceiroId:'PRC001', licId:l1.id, nomeCliente:'Marcelo Borges',   telefoneCliente:'(11) 91234-5678', emailCliente:'marcelo@gmail.com', cidadeCliente:'São Paulo', estadoCliente:'SP', produto:'Arrematação Individual', status:'NOVO',          origem:'lp_parceiro' },
    { id:'PL002', parceiroId:'PRC001', licId:l1.id, nomeCliente:'Tatiane Ferreira', telefoneCliente:'(11) 92345-6789', emailCliente:'tatiane@gmail.com', cidadeCliente:'Guarulhos', estadoCliente:'SP', produto:'Home Equity',           status:'EM_NEGOCIACAO',origem:'lp_parceiro', valorPrevisto:280000, valorReceber:2800 },
    { id:'PL003', parceiroId:'PRC002', licId:l1.id, nomeCliente:'Vivian Lopes',     telefoneCliente:'(11) 94567-8901', emailCliente:'vivian@gmail.com',  cidadeCliente:'Campinas',  estadoCliente:'SP', produto:'Revisão Contratual',     status:'NOVO',          origem:'lp_parceiro' },
    { id:'PL004', parceiroId:'PRC005', licId:l3.id, nomeCliente:'Sergio Mendonça',  telefoneCliente:'(31) 97890-1234', emailCliente:'sergio@gmail.com',  cidadeCliente:'Belo Horizonte', estadoCliente:'MG', produto:'Arrematação Individual', status:'FECHADO',  origem:'lp_parceiro', valorPrevisto:1100000, valorReceber:13200, fechadoEm: dt(-7) },
  ];
  for (const l of leads) {
    await prisma.partnerLead.upsert({ where:{id:l.id}, update:{}, create:l });
  }
  console.log(`  ${leads.length} partner-leads`);

  // ─── PARTNER PAYOUTS ────────────────────────────────────────
  const payouts = [
    { id:'PO001', parceiroId:'PRC001', licId:l1.id, leadId:'PL002', valorBase:280000,  tipoComissao:'PERCENTUAL', regraComissao:'INDICACAO',         valorCalculado:2800,  status:'APROVADO' },
    { id:'PO002', parceiroId:'PRC005', licId:l3.id, leadId:'PL004', valorBase:1100000, tipoComissao:'PERCENTUAL', regraComissao:'COMISSAO_RECEBIDA', valorCalculado:13200, status:'PENDENTE' },
  ];
  for (const p of payouts) {
    await prisma.partnerPayout.upsert({ where:{id:p.id}, update:{}, create:p });
  }
  console.log(`  ${payouts.length} partner-payouts`);
