...
router.post('/', auth, async (req, res, next) => {
  try {
    ...
    // Ownership: o cliente precisa existir e, para LIC, pertencer ao próprio tenant
    // (evita criar card apontando p/ cliente de outro licenciado e vazar nome/CPF).
    const cli = await prisma.cliente.findUnique({ where: { id: data.cliId }, select: { licId: true } });
    if (!cli) return res.status(404).json({ error: 'Cliente não encontrado.' });
    if (req.user.role === 'LIC' && cli.licId !== req.user.licId) {
      return res.status(403).json({ error: 'Acesso negado.' });
    }
    data.licId = cli.licId; // o card herda o tenant do cliente (consistência)
