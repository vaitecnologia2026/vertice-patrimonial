...
router.post('/', auth, adminOnly, async (req, res, next) => {
  try {
    ...
    if (data.role && !['ADMIN', 'LIC', 'COLAB'].includes(data.role)) {
      return res.status(400).json({ error: 'Papel inválido.' });
    }
    if (data.role === 'LIC' && !data.licId) {
      return res.status(400).json({ error: 'licId é obrigatório para usuário licenciado.' });
    }
...
// Reset de senha pelo ADMIN (não exige a senha atual) — remove dependência de suporte técnico.
router.patch('/:id/senha', auth, adminOnly, async (req, res, next) => {
  try {
    const { novaSenha } = req.body;
    if (!novaSenha || novaSenha.length < 8) {
      return res.status(400).json({ error: 'Nova senha mínima 8 caracteres.' });
    }
